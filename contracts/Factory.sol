...
contract Factory {
    ...
    function createCoop(string memory _name, string memory _symbol, uint32 _votingPeriod, uint32 _quorum, uint32 _supermajority, uint _membershipFee) public {
        require(existingSymbols[_symbol] == false, "duplicate symbol");
        existingSymbols[_symbol] = true;
        coop = new Coop(_name, _symbol, msg.sender, _votingPeriod, _quorum, _supermajority, _membershipFee);
